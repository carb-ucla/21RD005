---
title: "20240514_HCAI_ED_Cardiovascular_2015"
...
patterns_1 <- paste("^", sep = "", seq(410, 440, by = 1),
                    collapse = '|')

ED2015_1 <- subset((selectByDate(ED2015ALL, 
                                 month = c(1:9))), grepl(patterns_1, dx_prin))

patterns_2 <- paste(c(paste("^I", sep = "", seq(20, 25, by = 1)), 
...
ED2015_Cardio <- rbind(ED2015_1, ED2015_2)
ED2015_Cardio$Year <- 2015

ED2015_Cardio <- ED2015_Cardio %>% filter(patzip >= 90000)
...
ED2015_Cardio$agyrserv <- as.numeric(ED2015_Cardio$agyrserv, 
                                     ED2015_Cardio$patzip)

ED2015_Cardio$age <- ifelse(ED2015_Cardio$agyrserv < 20, '00_19',
                            ifelse(ED2015_Cardio$agyrserv >= 20 & 
                                     ED2015_Cardio$agyrserv < 45, '20_44',
                            ifelse(ED2015_Cardio$agyrserv >= 45 & 
                                     ED2015_Cardio$agyrserv < 65, '45_64',
                            ifelse(ED2015_Cardio$agyrserv >= 65, '65+',
                            'UK'))))

ED2015_Cardio$tcases <- 1

ED2015_Cardio <- subset(ED2015_Cardio, patzip >= 90000)

mED2015_Cardio <- setNames(aggregate(ED2015_Cardio$tcases, 
                                     list(ED2015_Cardio$Year, 
                                          ED2015_Cardio$patzip, 
                                          ED2015_Cardio$age, 
                                          ED2015_Cardio$sex), 
                                     FUN=sum), 
...
length(unique(mED2015_Cardio$zip)) * 4 * 2 * 3

unique_zip <- unique(mED2015_Cardio$zip)
unique_age <- unique(mED2015_Cardio$age)
unique_sex <- unique(mED2015_Cardio$sex)
unique_yr <- unique(mED2015_Cardio$year)
comb_zas15 <- expand.grid(year = unique_yr, zip = unique_zip, age = unique_age,
                          sex = unique_sex)
comb_zas15 <- subset(comb_zas15, sex != 'U')
comb_ED2015_Cardio <- merge(comb_zas15, mED2015_Cardio,
                            by = c("year", "zip", "age", "sex"), all = TRUE)
comb_ED2015_Cardio[is.na(comb_ED2015_Cardio)] <- 0
comb_ED2015_Cardio$outcome <- "ED_Cardio"
summary(comb_ED2015_Cardio$tcases)

ED2015_Cardio_AgeSex_US_tracking <- comb_ED2015_Cardio


# Save as CSV files

write.csv(ED2015_Cardio_AgeSex_US_tracking, file = 'C:/Users/VArputhasamy/Documents/HCAI_ED/Unsuppressed_Output/With_Zeros/Tracking_Data/ED2015_Cardio_AgeSex_US_fortracking.csv')
```
